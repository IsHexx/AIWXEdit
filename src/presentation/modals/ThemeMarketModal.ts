...
export class ThemeMarketModal extends Modal {
    ...
    private render() {
        ...
        contentEl.createEl('h2', { text: '主题市场' });
        ...
        // Spacer
        const spacer = tabContainer.createDiv({ cls: 'wdwxedit-market-spacer' });
        ...
        uninstallAllBtn.setButtonText('全部卸载')
            ...
            .onClick(() => {
                void this.uninstallAll(uninstallAllBtn);
            });
        ...
        batchBtn.setButtonText('全部下载')
            ...
            .onClick(() => {
                void this.downloadAll(batchBtn);
            });
    ...
    private createTab(container: HTMLElement, text: string, type: 'themes' | 'highlights') {
        ...
        tab.setCssProps({
            padding: '8px 16px',
            cursor: 'pointer',
            fontWeight: this.activeTab === type ? 'bold' : 'normal',
            borderBottom: this.activeTab === type ? '2px solid var(--interactive-accent)' : '2px solid transparent',
            color: this.activeTab === type ? 'var(--text-normal)' : 'var(--text-muted)',
        });
    ...
    private renderCard(container: HTMLElement, item: ThemeCatalogItem | HighlightCatalogItem) {
        ...
        info.setCssProps({ flex: '1', marginRight: '15px', minWidth: '0' });

        // Title & Author
        const header = info.createDiv({ cls: 'wdwxedit-card-header' });
        header.setCssProps({ marginBottom: '6px' });
        const titleEl = header.createDiv({ text: item.name, cls: 'wdwxedit-card-title' });
        titleEl.setCssProps({ fontWeight: 'bold', fontSize: '1.1em' });
        if (item.author) {
            const authorEl = header.createDiv({ text: `by ${item.author}`, cls: 'wdwxedit-card-author' });
            authorEl.setCssProps({ fontSize: '0.8em', color: 'var(--text-muted)' });
        }

        // Description
        const desc = info.createDiv({ cls: 'wdwxedit-card-desc' });
        desc.textContent = item.desc || (this.activeTab === 'highlights' ? '代码高亮样式' : '暂无描述');
        desc.setCssProps({ fontSize: '0.9em', color: 'var(--text-normal)', lineHeight: '1.4' });

        // Right: Action
        const action = card.createDiv({ cls: 'wdwxedit-card-action' });
        action.setCssProps({ minWidth: '80px', textAlign: 'right' });
